            ...
            for(String namebtn : tk.getMainFunction().getBtn().keySet()) {
                ...
                if(c.equals(tb)) {
                    ...
                    if("create".equals(namebtn)) {
                        TaiKhoanDialog tkdl = new TaiKhoanDialog(tk.getMf(),namebtn,tk,null);
                        tkdl.setVisible(true);
                    } else {
                        tkm = tk.getSelectedTaiKhoan();
                        if (tkm == null) {
                        ...
                        if("update".equals(namebtn) || "detail".equals(namebtn)) {
                            TaiKhoanDialog tkdl =  new TaiKhoanDialog(tk.getMf(),namebtn,tk,tkm);
                            tkdl.setVisible(true);
            ...
            if(tkdialog != null) {
                ...
                if(c instanceof JButton) {
                    if("Thêm".equals(btn)) {
                        ...
                        int  tt = tkdialog.getTrangThaiTextfield().getCombobox().getSelectedItem().toString().equalsIgnoreCase("Hoạt động") ? 1 : 0;
                        if(!Helper.isValidCode(manv) || !Helper.isValidCode(mk) || nqm == null) {
                            JOptionPane.showMessageDialog(null,"Vui lòng nhập đủ thông tin","thông báo",JOptionPane.ERROR_MESSAGE);
                            return;
                        }
                        tkm = new TaiKhoanModel(Integer.parseInt(manv),mk,tt,nqm);
                        ...
                        if(tkm != null) {
                            ...
                            tkm.setTrangthai((tkdialog.getTrangThaiTextfield().getCombobox().getSelectedItem().toString().equalsIgnoreCase("Hoạt Động") ? 1 : 0));
